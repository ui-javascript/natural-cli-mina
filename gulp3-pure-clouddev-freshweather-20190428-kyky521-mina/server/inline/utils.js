...
const KEY = 'hello'   //和风天气中应用的key名称
const USER_ID = 'HE1904281008361129' //和风天气中应用的密钥ID

const WECHAT_APPID = 'wxxxxxxxxxxxxxxxxx'
const WECHAT_APP_SECRET = '30xxxxxxxxxxxxxxxxxxxxxxxxxxxxxx'
const $ = {
  getWechatAppConfig: () => {
    return {
      id: WECHAT_APPID,
  ...
  getWeatherImage(code, isNight) {
    // const bgUrl = WEATHER_IMAGE_PERFIXER + '/' + path.join(isNight ? 'night' : 'day', `${code}.svg`)
    const bgUrl = WEATHER_IMAGE_PERFIXER + '/' + isNight ? 'night' : 'day' + `/${code}.svg`
    return bgUrl
  },
  getBackgroundImage(weather, isNight) {
    // const imgUrl = BACKGROUND_PERFIXER + '/' + path.join(isNight ? 'night' : 'day', `${weather}.jpg`)
    const imgUrl = BACKGROUND_PERFIXER + '/' + (isNight ? 'night' : 'day') + `/${weather}.jpg`
    console.log("@@@@" + imgUrl)
    return imgUrl
  },
